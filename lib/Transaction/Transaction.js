 ...
 * @returns {Transaction}
 */
Transaction.prototype.signCovenant = function (inputIndex, privKeysOrScripts, subscript, anyoneCanSpend = false) {
    let sighash = (Signature.SIGHASH_ALL | Signature.SIGHASH_FORKID) // Must be ALL in order for covenant to function
    let sigScripts = []
        ...
        if (sigScript instanceof PrivateKey) {
            ...
            sigScript = this.getCovenantSignatureScript(inputIndex, privKey, subscript, sighash)
                ...
                if (privKeysOrScripts.length > 1) {
                    throw new Error('For anyoneCanSpend inputs, prvKeysOrScripts must have a length of 1 and consist of a single PrivateKey')
                }
                let input = this.toObject()['inputs'][inputIndex]
